...
# def save_gamma_videos(gamma, fr_trj, save_dir, file_name, scale=4):
#     """
#     gamma: [timestep x channel x h x w] のサイズの動画データ
#     fr_trj: firing rate trajectory, list of lists with length equal to timesteps
#     save_dir: 保存先ディレクトリ
#     file_name: ファイル名
#     scale: 描画領域のスケール倍率
#     """
...
#     timesteps, channels, height, width = gamma.shape
#     grid_size = int(np.ceil(np.sqrt(channels)))  # グリッドサイズを計算

#     # ヒートマップの色範囲を設定
#     vmin, vmax = np.min(gamma), np.max(gamma)
...
#     frame_size = (grid_size * width * scale + 50, grid_size * height * scale + 100)  # 50 pixels for color bar and labels, 100 pixels for fr_trj
#     out = cv2.VideoWriter(tmpout, fourcc, 30.0, frame_size)

#     # カラーバーの作成
#     color_bar = np.zeros((grid_size * height * scale, 50, 3), dtype=np.uint8)
#     for i, label in enumerate(np.linspace(vmin, vmax, num=color_bar.shape[0])):
#         color = cv2.applyColorMap(np.array([[int(255 * (label - vmin) / (vmax - vmin))]], dtype=np.uint8), cv2.COLORMAP_JET)[0][0]
#         color_bar[i, :] = color

#     # 各チャンネルごとの動画を作成
#     for t in range(timesteps):
#         frame = np.zeros((grid_size * height * scale + 100, grid_size * width * scale + 50, 3), dtype=np.uint8)
#         for c in range(channels):
#             row = c // grid_size
#             col = c % grid_size
#             heatmap = cv2.normalize(gamma[t, c], None, 0, 255, cv2.NORM_MINMAX)
#             heatmap = cv2.applyColorMap(heatmap.astype(np.uint8), cv2.COLORMAP_JET)
#             heatmap = cv2.resize(heatmap, (width * scale, height * scale), interpolation=cv2.INTER_NEAREST)
#             frame[row * height * scale:(row + 1) * height * scale, col * width * scale:(col + 1) * width * scale] = heatmap

#         # カラーバーをフレームに追加
#         frame[:grid_size * height * scale, -50:] = cv2.resize(color_bar, (50, grid_size * height * scale), interpolation=cv2.INTER_NEAREST)

#         # 数値ラベルを追加
#         for i, label in enumerate(np.linspace(vmin, vmax, num=10)):
#             y = int((1 - i / 9) * grid_size * height * scale)
#             cv2.putText(frame, f'{label:.2f}', (frame.shape[1] - 40, y), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1, cv2.LINE_AA)

#         # fr_trjを描画
#         fr_trj_img = np.zeros((100, grid_size * width * scale + 50, 3), dtype=np.uint8)
#         fr_t = [fr_trj[step] for step in range(t)]
#         fr_t_none = [None for _ in range(len(fr_trj) - t - 1)]
#         for i in range(1, len(fr_t)):
#             if fr_t[i - 1] is not None and fr_t[i] is not None:
#                 cv2.line(fr_trj_img, (int((i - 1) * (grid_size * width * scale) / timesteps), 100 - int(fr_t[i - 1] * 100 / max(fr_trj))),
#                          (int(i * (grid_size * width * scale) / timesteps), 100 - int(fr_t[i] * 100 / max(fr_trj))), (255, 255, 255), 1)

#         # y軸の数値を描画
#         for i, label in enumerate(np.linspace(0, max(fr_trj), num=5)):
#             y = int((1 - i / 4) * 100)
#             cv2.putText(fr_trj_img, f'{label:.2f}', (grid_size * width * scale + 5, y), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1, cv2.LINE_AA)

#         # fr_trjをフレームに追加
#         frame[grid_size * height * scale:, :grid_size * width * scale] = fr_trj_img[:, :grid_size * width * scale]
